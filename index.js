...
const dynamicProxy = createProxyMiddleware({
  ...
  on: {
    error: (err, req, res) => {
      console.error('[ERROR DE PROXY]', err && err.message);
      try {
        ...
        console.error('[ERROR ADICIONAL ENVIANDO RESPUESTA DE ERROR]', e);
      }
    }
  }
});
...
app.get('/health', (req, res) => {
  res.json({ ok: true, activeTunnelsCount: Object.keys(activeTunnels).length, uptimeSeconds: process.uptime() });
});
...
// Protección: si una sola petición produce un error no controlado (por
// ejemplo, un problema puntual reenviando hacia una PC), esto evita que
// TODO el servidor se caiga por eso — solo se registra el error en el
// log y el servidor sigue atendiendo al resto de las PCs normalmente.
process.on('uncaughtException', (err) => {
  console.error('[ERROR NO CONTROLADO — el servidor sigue funcionando]', err);
});
process.on('unhandledRejection', (reason) => {
  console.error('[PROMESA RECHAZADA NO CONTROLADA — el servidor sigue funcionando]', reason);
});
